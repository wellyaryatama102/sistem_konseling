...
<div style="max-width:650px; margin:0 auto;">
    <div style="margin-bottom:1.5rem;">
        <h2 style="margin:0; font-size:1.5rem; font-weight:800; color:var(--primary-dark);">Edit Kelas</h2>
        <p style="color:var(--text-muted); margin:0.25rem 0 0 0; font-size:0.875rem;">Perbarui informasi kelas {{ $kelas->nama_kelas }}.</p>
    </div>
            ...
            <div class="grid-2" style="gap:1rem; grid-template-columns: 1fr 1fr; margin-bottom:1rem;">
                <div>
                    ...
                    <select name="tingkat_kelas" class="form-control" required>
                        <option value="X" {{ old('tingkat_kelas', $kelas->tingkat_kelas) == 'X' ? 'selected' : '' }}>Tingkat X</option>
                        <option value="XI" {{ old('tingkat_kelas', $kelas->tingkat_kelas) == 'XI' ? 'selected' : '' }}>Tingkat XI</option>
                        <option value="XII" {{ old('tingkat_kelas', $kelas->tingkat_kelas) == 'XII' ? 'selected' : '' }}>Tingkat XII</option>
                    </select>
                </div>
            ...
            <div class="form-group">
                ...
                <select name="id_tahun_ajaran" class="form-control" required>
                    ...
                    @foreach($tahunAjarans as $ta)
                        <option value="{{ $ta->id_tahun_ajaran }}" {{ old('id_tahun_ajaran', $kelas->id_tahun_ajaran) == $ta->id_tahun_ajaran ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <select name="id_wali_kelas" class="form-control">
                    ...
                    @foreach($waliKelases as $w)
                        <option value="{{ $w->id_wali_kelas }}" {{ old('id_wali_kelas', $kelas->id_wali_kelas) == $w->id_wali_kelas ? 'selected' : '' }}>
